...
class UserForm extends Component {
  ...
  handleSelectedFiles = e => {
    const now = new Date();
    const dates = `${this.timeFormat(now.getDate())}/${this.timeFormat(now.getMonth() + 1)}/${now.getFullYear()}`;
    const times = `${this.timeFormat(now.getHours())}:${this.timeFormat(now.getMinutes())}:${this.timeFormat(now.getSeconds())}`;
    let newFiles = [];
  ...
  validate = (name, address, email, telnum) => {
    ...
    if (touchedTelnum && !telnumPattern.test(telnum)) errors.telnum = 'phone number only in digits';
